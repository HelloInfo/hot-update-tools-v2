    ...
    ready() {
        ...
        window.plugin = new window.Vue({
            ...
            data: {
                ...
                versionDebug: "",
                remoteServerVersionDebug: "",
                isShowRemoteServerVersionDebug: false,// 是否显示远程服务器debug版本号
                genManifestDirDebug: "",
                serverRootDirDebug: "",
                ...
                localGameVersionDebug: "-",//游戏版本号
                localGamePackageUrlDebug: "",
                ...
                localGameProjectManifestDebug: "",
                localGameVersionManifestDebug: "",
                localGameProjectManifestUrlDebug: "",//assets的project.manifest配置的url
                localGameVersionManifestUrlDebug: "",//assets的version.manifest配置的url
            ...
            methods: {
                _initPluginCfg() {
                    ...
                    // manifest输出目录
                    this.genManifestDir = CfgUtil.getMainFestDir("release");
                    if (FileUtil.isFileExit(this.genManifestDir) === false) {
                        FileUtil.mkDir(this.genManifestDir);
                    }
                    this.genManifestDirDebug = CfgUtil.getMainFestDir("debug");
                    if (FileUtil.isFileExit(this.genManifestDirDebug) === false) {
                        FileUtil.mkDir(this.genManifestDirDebug);
                    }
                    ...
                    CfgUtil.initCfg(function (data) {
                        if (data) {
                            ...
                            this.versionDebug = data.versionDebug
                            this.serverRootDirDebug = data.serverRootDirDebug;

                            this._updateServerVersion();
                            this._getRemoteServerVersion();
                            this._getRemoteServerVersionDebug();
                        } else {
                            this._saveConfig();
                        }
                        this._initResourceBuild();
                        this.initLocalGameVersion();
                        this.initLocalGameVersionDebug();
                ...
                _saveConfig() {
                    let data = {
                        ...
                        genManifestDir: CfgUtil.getMainFestDir("release"),
                        localServerPath: this.localServerPath,
                        hotAddressArray: this.hotAddressArray,

                        versionDebug: this.versionDebug,
                        serverRootDirDebug: this.serverRootDirDebug,
                        genManifestDirDebug: CfgUtil.getMainFestDir("debug"),
                    };
                ...
                onBtnClickTellMe() {
                    let url = "http://wpa.qq.com/msgrd?v=3&uin=825808091&site=qq&menu=yes";
                    Electron.shell.openExternal(url);
                },
                ...
                // 将当前版本打包
                _packageVersion(buildType) {
                    this._addLog("[Pack] 开始打包版本 ...");
                    let JSZip = Editor.require("packages://hot-update-tools/node_modules/jszip");
                    let zip = new JSZip();

                    let buildTypeVersion = "";
                    let manifestDirPath = "";
                    if (buildType == "debug") {
                        buildTypeVersion = this.versionDebug;
                        manifestDirPath = this.genManifestDirDebug;
                    } else {
                        buildTypeVersion = this.version;
                        manifestDirPath = this.genManifestDir;
                    }

                    // 打包manifest文件
                    let version = path.join(manifestDirPath, "version.manifest");
                    zip.file("version.manifest", fs.readFileSync(version));
                    let project = path.join(manifestDirPath, "project.manifest");
                    ...
                    if (versionStr !== buildTypeVersion) {
                ...
                // 选择项目的manifest文件目录
                onSelectProjectManifestDir(buildType) {
                    ...
                    if (res !== -1) {
                        ...
                        this.onImportManifestToGame(buildType);
                        this._addLog("导入完成,请检查项目目录:" + url);
                    }
                },

                // 导入生成的manifest到游戏项目中
                onImportManifestToGame(buildType) {
                    let locPath = "";
                    let locURL = "";
                    if (buildType == "debug") {
                        locPath = this.genManifestDirDebug;
                        locURL = this.localGameProjectManifestUrlDebug
                    } else {
                        locPath = this.genManifestDir;
                        locURL = this.localGameProjectManifestUrl
                    }
                    let projectFile = path.join(locPath, "project.manifest");
                    let versionFile = path.join(locPath, "version.manifest")
                    ...
                    let strArr = locURL.split("project.manifest");
                    let dir = strArr[0];
                ...
                initLocalGameVersion() {
                    ...
                    Editor.assetdb.queryAssets('db://assets/resources/manifest/release/**\/*', "raw-asset", function (err, results) {
                        let versionCfg = "";
                        let projectCfg = "";
                        results.forEach(function (result) {
                            if (result.path.indexOf("project.manifest") !== -1) {
                                projectCfg = result.path;
                                this.localGameProjectManifestUrl = result.url;
                            } else if (result.path.indexOf("version.manifest") !== -1) {
                ...
                initLocalGameVersionDebug() {
                    // Editor.assetdb.queryAssets('db://assets/**\/*', "raw-asset", function (err, results) {
                    Editor.assetdb.queryAssets('db://assets/resources/manifest/debug/**\/*', "raw-asset", function (err, results) {
                        let versionCfg = "";
                        let projectCfg = "";
                        results.forEach(function (result) {
                            if (result.path.indexOf("project.manifest") !== -1) {
                                projectCfg = result.path;
                                this.localGameProjectManifestUrlDebug = result.url;
                            } else if (result.path.indexOf("version.manifest") !== -1) {
                                versionCfg = result.path;
                                this.localGameVersionManifestUrlDebug = result.url;
                            }
                        }.bind(this));
                        console.log("project: " + projectCfg);
                        console.log("version: " + versionCfg);
                        ...
                        if (versionCfg.length === 0) {
                            this._addLog("[测试服]项目中没有配置文件: version.manifest");
                            return;
                        }
                        if (projectCfg.length === 0) {
                            this._addLog("[测试服]项目中没有配置文件: project.manifest");
                            return;
                        }

                        this.localGameProjectManifestDebug = projectCfg;
                        this.localGameVersionManifestDebug = versionCfg;
                            ...
                            if (!err) {
                                ...
                                fs.readFile(projectCfg, 'utf-8', function (err, data) {
                                    if (!err) {
                                        ...
                                        projVersion = projectData.version;
                                        if (projVersion === verVersion) {
                                            this.localGameVersionDebug = projVersion;
                                            this.localGamePackageUrlDebug = projectData.packageUrl;
                                        } else {
                                            this._addLog("[测试服]游戏中的 project.manifest 和 version.manifest 中的version字段值不一致,请检查配置文件");
                                        }
                                    } else {
                                        // this.localGameVersion = "没有在项目中发现热更新文件";
                                        this._addLog("[测试服]读取项目中的配置文件失败: " + projectCfg);
                                    }
                                }.bind(this))
                            } else {
                                this._addLog("[测试服]读取项目中的配置文件失败: " + versionCfg);
                            }
                        }.bind(this))
                    }.bind(this));
                },
                ...
                onClickGenCfg(event) {
                    // 检查是否需要构建项目
                    this._addLog("");
                    this._addLog("[正式服] onClickGenCfg()");
                    ...
                    if (genTime === buildTime) {// 构建完版本之后没有生成manifest文件
                        CfgUtil.updateGenTime(new Date().getTime(), this.version, "release");// 更新生成时间
                    } else {
                        this._addLog("[正式服] 你需要重新构建项目,因为上次构建已经和版本关联:" + CfgUtil.cfgData.genVersion);
                        return;
                    }

                    if (!this.version || this.version.length <= 0) {
                        this._addLog("[正式服] 版本号未填写");
                        return;
                    }
                    if (!this.serverRootDir || this.serverRootDir.length <= 0) {
                        this._addLog("[正式服] 服务器地址未填写");
                        return;
                    }

                    // 检查resource目录
                    if (this.resourceRootDir.length === 0) {
                        this._addLog("[正式服] 请先指定 <build项目资源文件目录>");
                        return;
                    }
                    ...
                    if (!this.genManifestDir || this.genManifestDir.length <= 0) {
                        this._addLog("[正式服] manifest文件生成地址未填写");
                        return;
                    }

                    if (!fs.existsSync(this.genManifestDir)) {
                        this._addLog("[正式服] manifest存储目录不存在: " + this.genManifestDir);
                        return;
                    }

                    this._saveConfig();
                    this._genVersion("release", this.version, this.serverRootDir, this.resourceRootDir, this.genManifestDir);
                },

                onClickGenCfgDebug(event) {
                    // 检查是否需要构建项目
                    this._addLog("");
                    this._addLog("[测试服] onClickGenCfgDebug()");
                    let times = CfgUtil.getBuildTimeGenTime();
                    let genTime = times.genTime;
                    let buildTime = times.buildTime;
                    if (genTime === buildTime) {// 构建完版本之后没有生成manifest文件
                        CfgUtil.updateGenTime(new Date().getTime(), this.versionDebug, "debug");// 更新生成时间
                    } else {
                        this._addLog("[测试服] 你需要重新构建项目,因为上次构建已经和版本关联:" + CfgUtil.cfgData.genVersionDebug);
                        return;
                    }

                    if (!this.versionDebug || this.versionDebug.length <= 0) {
                        this._addLog("[测试服]版本号未填写");
                        return;
                    }
                    if (!this.serverRootDirDebug || this.serverRootDirDebug.length <= 0) {
                        this._addLog("[测试服] 服务器地址未填写");
                        return;
                    }

                    // 检查resource目录
                    if (this.resourceRootDir.length === 0) {
                        this._addLog("[测试服] 请先指定 <build项目资源文件目录>");
                        return;
                    }
                    if (this._checkResourceRootDir(this.resourceRootDir) === false) {
                        return;
                    ...
                    if (!this.genManifestDirDebug || this.genManifestDirDebug.length <= 0) {
                        this._addLog("[测试服] manifest文件生成地址未填写");
                        return;
                    }

                    if (!fs.existsSync(this.genManifestDirDebug)) {
                        this._addLog("[测试服] manifest存储目录不存在: " + this.genManifestDirDebug);
                        return;
                    }

                    this._saveConfig();
                    this._genVersion("debug", this.versionDebug, this.serverRootDirDebug, this.resourceRootDir, this.genManifestDirDebug);
                },

                // serverUrl 必须以/结尾
                // buildResourceDir 默认为 build/jsb-default/
                // manifestDirPath 建议在assets目录下
                _genVersion(buildType, version, serverUrl, buildResourceDir, manifestDirPath) {
                    ...
                    this._packageVersion(buildType);
                },
                ...
                onCopyFileToLocalServer() {
                    ...
                    // 检查manifest文件
                    let project = path.join(this.genManifestDirDebug, "project.manifest");
                    let version = path.join(this.genManifestDirDebug, "version.manifest");
                    if (!this.genManifestDirDebug || this.genManifestDirDebug.length <= 0) {
                ...
                onInputVersionOver(buildType) {
                    this._addLog("onInputVersionOver() >>> " + buildType + " , versionDebug = " + this.versionDebug);

                    let buildTypeVersion = "";
                    let buildVersion = "";
                    if (buildType == "release") {
                        buildTypeVersion = this.version
                        buildVersion = CfgUtil.cfgData.genVersion;
                    } else if (buildType == "debug") {
                        buildTypeVersion = this.versionDebug
                        buildVersion = CfgUtil.cfgData.genVersionDebug;
                    }
                    ...
                    if (remoteVersion !== null && remoteVersion !== undefined) {// 存在远程版本
                        if (this._isVersionPass(buildTypeVersion, remoteVersion)) {
                            this._addLog("上次构建时版本号: " + buildVersion);
                            if (this._isVersionPass(buildTypeVersion, buildVersion)) {// 上次构建版本和远程版本一致
                                this._addLog("版本通过验证!");
                            } else {
                                this._addLog("[Warning] 要构建的版本低于上次构建版本: " + buildTypeVersion + "<=" + buildVersion);
                            }
                        } else {
                ...
                onInPutUrlOver(buildType) {
                    this._addLog("您输入的URL类型是 = " + buildType);
                    let url = "";
                    if (buildType == "release") {
                        url = this.serverRootDir;
                    } else if (buildType == "debug") {
                        url = this.serverRootDirDebug;
                    }
                    ...
                    if (index1 === -1 && index2 === -1) {
                        ...
                        if (!reg.test(url)) {
                            ...
                            if (buildType == "release") {
                                ...
                                this.serverRootDirDebug = "http://" + this.serverRootDirDebug;
                            }
                            this._getRemoteServerVersion();
                            this._getRemoteServerVersionDebug();
                        }
                    ...
                    if (buildType == "release") {
                        if (this.serverRootDir != "http://") {
                            this._addHotAddress(this.serverRootDir);
                        }
                    } else if (buildType == "debug") {
                        // this._addHotAddress(this.serverRootDirDebug);
                    }
                    this._updateShowUseAddrBtn();
                    this._saveConfig();
                },
                ...
                // 获取远程资源服务器Debug的版本号
                _getRemoteServerVersionDebug() {
                    if (this.serverRootDirDebug.length <= 0) {
                        console.log("远程资源服务器URL错误: " + this.serverRootDirDebug);
                        return;
                    }
                    // TODO 浏览器缓存会导致版本号获取失败
                    this.isShowRemoteServerVersionDebug = false;
                    this.remoteServerVersionDebug = null;
                    let versionUrl = this.serverRootDirDebug + "/version.manifest";
                    ...
                    xhr.onreadystatechange = function () {
                        if (xhr.readyState === 4 && ((xhr.status >= 200 && xhr.status < 400))) {
                            ...
                            this.isShowRemoteServerVersionDebug = true;
                            this.remoteServerVersionDebug = json.version;
                        } else if (xhr.status === 404) {
                            // console.log("404");
                        }
                    }.bind(this);
                ...
                userLocalIP(buildType) {
                    ...
                    if (ip.length > 0) {
                        if (buildType == "release") {
                            ...
                            this.serverRootDirDebug = "http://" + ip;
                        }
                ...
                onOpenManifestDir(buildType) {
                    let url = "";
                    if (buildType == "debug") {
                        url = this.genManifestDirDebug;
                    } else {
                        url = this.genManifestDir;
                    }
                    if (!fs.existsSync(url)) {
                        this._addLog("目录不存在：" + url);
                        return;
                    }
                ...
                onOpenLocalGameManifestDir(buildType) {
                    let strArr = "";
                    if (buildType == "debug") {
                        strArr = this.localGameProjectManifestDebug.split("project.manifest");
                    } else {
                        strArr = this.localGameProjectManifest.split("project.manifest");
                    }
